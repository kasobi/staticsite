...
def r_generate_pages(basepath, from_path, template_path, dest_path):
    if os.path.isdir(from_path):
        if not os.path.exists(dest_path):
            os.mkdir(dest_path)
            print(f"made directory: {dest_path}")
        for path in os.listdir(from_path):
            r_generate_pages(basepath, os.path.join(from_path, path), template_path, os.path.join(dest_path, path))
